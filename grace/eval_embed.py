    ...
    for s in Config.NAMES:
        for t in Config.NAMES:
            if (t != s and 
                        ...
                        for jj, amp in enumerate(amp_list):
                            ...
                            if params['iteration_flag']:
                                for it in range(100, 1000, 100):
                                    adv_img_file = '{}_{}_{}_loss_{}_{}_marg_{}_it_{}_amp_{}.png'.format(attack_name,
                                                                                                   model_name,
                                                                                                   attack_loss[0],
                                                                                                   source_file,
                                                                                                   t,
                ...
                it_db.extend(iterations)
    return face_db, name_db, margin_db, amp_db, people_db, it_db
...
def whitebox_eval(embedding_means,
    ...
    final_labels, final_distances, final_labels_cos, final_cosines = return_top(final, final_cos, topn)
    return final_labels, final_distances, final_labels_cos, final_cosines
...
if __name__ == '__main__':
    # folder = '../facenet/datasets/lfw-aligned-96'
    tf_config = Config.set_gpu('0')
    params = Config.set_parameters(model_type='small',
                                   loss_type='center',
                                   dataset_type='vgg',
                                   batch_size=20)
    # faces, people = load_images(folder, params)
    # embedding_means, _ = compute_embeddings(faces, people, tf_config, params)
    # people_db = []
    # embedding_db = []
    # for person, mean in embedding_means.items():
    #     people_db.append(person)
    #     embedding_db.append(mean)
    # np.savez(os.path.join(Config.ROOT, 'embeddings/lfw_embeddings.npz'), people=np.array(people_db), mean=np.array(embedding_db))
    npzfile = np.load(os.path.join(Config.ROOT, 'embeddings/lfw_embeddings.npz'))
    people = npzfile['people']
    means = npzfile['mean']
    pairs = {}
    for p, m in zip(people, means):
        min_distance = 1e10
        min_person = p
        for pi, mi in zip(people, means):
            if not p == pi:
                cos_sim = np.dot(m, mi) / (np.linalg.norm(m) * np.linalg.norm(mi))
                distance = np.linalg.norm(m - mi)
                ...
                if avg < min_distance:
                    ...
                    min_person = pi
        pairs[p] = min_person
        print(p, min_person, min_distance)